...
void CameraInterface::publishImage(const std::vector<AVBufferRef *>& planes, const std::vector<unsigned int>& strides, uint buffer_size, long timestamp_ns, bool log) {
    ...
    switch (this->image_output_format) {
        ...
        //     this->out_image_msg.data.assign(rgb.data, rgb.data + (rgb.rows * rgb.cols * 3));
        
        //     this->out_image_msg.step = this->width * 3;  // ✅ Fix for stride issue
        
        //     // Debug log
        //     // this->log(GREEN, " >> Sending Image step: ", this->out_image_msg.step, CLR);
        // }
        // break;
